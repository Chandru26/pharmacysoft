'use strict';
app.controller('InvoicesController', function ($scope,$http,$timeout, uiGridConstants, $uibModal, $log, Upload, Flash,deemsoft)
{	var csrf = document.getElementById("csrf_token").value;
	...
	$scope.getMaxInvoiceID = function(){
		deemsoft.getHTTP('../invoicesrest/getmaxid/')
	...
	$scope.getInvoice = function(){
		...
		.then(function(r){	
			$scope.Invoice = r; 
			$scope.Invoice_id = $scope.Invoice.id;
			...
			$scope.discount = parseFloat($scope.Invoice.discount).toFixed(2);
			...
			$scope.orderDate = deemsoft.IndianDate($scope.Invoice.invoice_date);
	...
	$scope.addToList = function(id,barcode,gst,name,expiration,msrp,purchase_price,discount,tax,qty){
		...
		$scope.catalogItem ={id:'',catalogs_id:'',invoices_id:'',barcode:'',name:'', expiration:'',price:'',purchase_price:'',quantity:0,gst:'',tax:'',discount:'',subtotal:''};
 ...
 $scope.saveInvoice = function(){
	...
	}else
	{
		...
		$scope.Invoice.title = $scope.contactName;
		...
		$scope.Invoice.total = parseFloat($scope.grandTotal).toFixed(0);
		...
		$scope.Invoice.sgst_amount = $scope.sgst;			
		$scope.Invoice.cgst_amount = $scope.cgst;
		
		deemsoft.postHTTP('../invoicesrest/saveinvoices/', $scope.Invoice,csrf)
		.then(function(inv){	
			$scope.Invoice_id = inv.id;	
			$scope.print = false;		
			for(var i = 0; i < $scope.InvoiceTrxs.length; i++){
				$scope.Invoice = inv.id;
				$scope.InvoiceTrxs[i].invoices_id = inv.id;
 ...
 $scope.printInvoice = function(){
			...
			// var htmlData = "<html><head><link rel=\"stylesheet\" href=\"http://localhost:8080/pharmacysoft/static/css/bootstrap/bootstrap.min.css\">";  
			 var htmlData = "<html><head><link rel=\"stylesheet\" href=\"../static/css/bootstrap/bootstrap.min.css\">";  
			...
			"<td style=\"morgin-left:15px\">"+
				...
				htmlData += "<tr><td>Patient Name:<b> "+$scope.contactName+"</b></td><td style=\"text-align:right;\">Invoice No:<b> "+$scope.Invoice_id+"</b></td></tr>";
